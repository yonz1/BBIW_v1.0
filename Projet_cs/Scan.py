...
parser.add_argument("--directory", help="Provide a directory", action="store_true")
...
if args.imported:
    ...
    if args.directory:
        ...
        print(str(argument))
        all_files = [f for f in pathlib.Path(argument).iterdir() if f.is_file()] 
        print(len(all_files))
        file_i.write("Total files in directory : " + str(len(all_files)) + "\n")
        for file in all_files:
            a = 0
            file_i.write("Selected file : " + str(file) + "\n")
            file = str(file)
            file = file.replace("C:\\", "/")
            cur_file = file.replace("\\", "/")
            print(cur_file)
            for path_rule in files:
                try:
                    path_rule = str(path_rule)
                    path_rule = path_rule.replace("\\", "/")
                    print(path_rule) 
                    rules = yara.compile(path_rule)
                    ...
                    ### Matches 
                    matches = rules.match(cur_file, warnings_callback=warning_callback, timeout=30)
                    i = len(matches)
                    ...
                    file_i.write(str(e) + "\n")  
            file_i.write("Number of detected threat in this file : " + str(a) + "\n")
        ...
        for path_rule in files:
            try :
                ...
                file_i.write(e)
file_i.write("\n" + "\n" + "Total number of detected threat : " + str(z) + "\n")
...
if args.predefined:
    totfiles = ["C:/Users/Bastien/Documents/", "C:/Users/Bastien/Desktop/", "C:/Users/Bastien/Downloads", "C:/Users/Bastien/Pictures"]
    for file in totfiles:
        all_files = [f for f in pathlib.Path(file).iterdir() if f.is_file()]
        for x in all_files:
            x = str(x)
            x = x.replace("C:\\", "/")
            x = x.replace("\\", "/")
